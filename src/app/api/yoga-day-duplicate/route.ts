...
export async function POST(request: NextRequest) {
    try {
        ...
        const phoneQuerySnapshot = await firestore.collection('arambhaForm26').where('phone', '==', phone).limit(1).get()
        ...
        const emailQuerySnapshot = await firestore.collection('arambhaForm26').where('email', '==', email).limit(1).get()
